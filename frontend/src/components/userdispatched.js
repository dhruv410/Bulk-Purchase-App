...
/* App component */
class UserDispatched extends Component {
  ...
  onReview(e,index){
    ...
    axios.post('http://localhost:4000/orderreview', user)
    .then(res => {
        console.log(res.data)
        if(res.data.message === 'Reviewed')
        {
            // this.props.history.push('/productstatus')
            // window.location.reload()
        }
  ...
  render() {
      ...
      else if (item.vendorrated == 0 && item.orderrated != 0) {return (
        <div>
        <li>
      ...
      <br></br>
        ...
        <form className={classes.form} noValidate>
          <TextField
            ...
            value = {this.state.rate[index]}
            // onChange = {this.onChangeQuantity[index]}
            onChange = {(event) => this.onChangeReview(event,this.state.rate[index])}
          />
          </form>
          <Button
        type="submit"
        ...
        color="primary"
        className={classes.submit}
        onClick = {(event) => this.onReview(event,item)}
         >
        Review Order
      </Button>
      <br></br>
      ...
      else if (item.vendorrated!=0&&item.orderrated!=0)
      {
        return (
        <div>
        <li>
...
export default  withStyles(styles)(UserDispatched);